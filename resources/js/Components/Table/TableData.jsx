    ...
    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400 table-fixed">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                {data[0] != null && Object.keys(data[0]).map((x, i) => 
                    i > 0 && <th key={'col-'+ i + '-header'} scope="col" className={`w-${width[i]} px-4 py-3`}>{x}</th>
                )}
        ...
        <tbody>
            {[...Array(10)].map((e,i) => {if(i < data.length)
                ...
                return <tr key={'row'+i} className={"border-b dark:border-gray-700"}>
            ...
            <td className="px-4 py-3">{data[i].status <= 0 ? statusDict.get(data[i].status):  data[i].status}</td>*/}
                    <td key={'row-'+i+'actions'} className="px-4 py-3 flex items-center justify-end">
